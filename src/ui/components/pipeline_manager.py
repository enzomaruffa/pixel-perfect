...
def render_save_pipeline(has_pipeline: bool):
    ...
    with st.form("save_pipeline_form", clear_on_submit=False):
        pipeline_name = st.text_input(
            "Pipeline Name", value="My Pipeline", help="Name for your saved pipeline"
        )

        pipeline_description = st.text_area(
            "Description (Optional)", value="", help="Brief description of what this pipeline does"
        )
        ...
        if save_clicked:
            try:
                ...
                filename = (
                    f"{safe_name or 'pipeline'}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                )
...
def render_load_pipeline():
    ...
    uploaded_file = st.file_uploader(
        "Upload Pipeline", type=["json"], help="Upload a previously saved pipeline JSON file"
    )
...
def load_pipeline_to_session(pipeline_data: dict[str, Any], replace: bool = True):
    ...
    try:
        ...
        raise Exception(f"Failed to load pipeline: {str(e)}") from e
