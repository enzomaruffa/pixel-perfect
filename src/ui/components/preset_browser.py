...
def load_preset_to_pipeline(preset_config: Dict[str, Any], replace: bool = True, preset_name: str = ""):
...
def convert_preset_to_pipeline(preset_config: Dict[str, Any], preset_name: str = "") -> List[Dict[str, Any]]:
    ...
    for i, op_config in enumerate(operations):
        ...
        if preset_name:
            display_name = f"{op_type} ({preset_name})"
        else:
...
def render_preset_quick_load():
    ...
    display_names = [name.replace('-', ' ').title() for name in preset_names]
