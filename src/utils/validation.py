...
def validate_expression_safe(expression: str) -> None:
    ...
    allowed_vars = {
        ...
        "g",
        "b",
        "a",  # Color channels (0-255)
        "x",
        "y",  # Spatial coordinates (x=column, y=row)
        "width",
    ...
    for func in functions_used:
        if (
            func.lower() not in allowed_functions
            and func.lower() not in allowed_vars
            and func.lower() not in deprecated_vars
        ):
    ...
    for var in variables_used:
        if var in deprecated_vars and var == "j":
            warnings.warn(
                (
